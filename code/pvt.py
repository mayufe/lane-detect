...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, patch_size_y=False):
        super().__init__()
        ...
        if patch_size_y:
            self.num_patches = self.H * self.W//4
        else:
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=1024, patch_size=16, in_chans=32, num_classes=1000, embed_dims=[64, 128, 256, 512],
    ...
    def forward_features(self, x):
        ...
        # print('pos_embed1:', self.pos_embed1.shape)
        x = x + self.pos_embed1
        # dropout
        x = self.pos_drop1(x)
        ...
        x4 = x4 + self.pos_embed4
        x4 = self.pos_drop4(x4)
...
def pvt_tiny(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=2, embed_dims=[64, 128, 256, 512], in_chans=64, num_heads=[1, 2, 4, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
def pvt_small(pretrained=False, **kwargs):
    model = PyramidVisionTransformer(
        patch_size=2, embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
if __name__ == "__main__":
    ...
    n = pvt_tiny()
    img = torch.randn(1, 32, 1024, 1024)
    out = n(img)
    print(out[0].shape)
    print(type(out))
